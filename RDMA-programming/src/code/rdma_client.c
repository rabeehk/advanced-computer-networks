...
static int client_remote_memory_ops() 
{
	...
	/* add the local address to the sge */
	client_send_sge.addr = (uint64_t)src;
	client_send_sge.length = (uint32_t)strlen(src);
	client_send_sge.lkey = client_src_mr->lkey;
	bzero(&client_send_wr, sizeof(client_send_wr));

	/* create the worklist with the remote server address */
	client_send_wr.sg_list = &client_send_sge;
	client_send_wr.num_sge = 1;
	...
	if (ret) {
		rdma_error("Failed to send write request, ret: %d\n", ret);
		return ret;
	}
	...
	ret = process_work_completion_events(io_completion_channel, 
			&wc, 1);
	if (ret != 1) {
		rdma_error("error when writing to remote, ret = %d\n", -errno);
	...
	if (!client_dst_mr) {
		rdma_error("Failed to register client destination buffer, ret = %d\n", -errno);
		return -errno;
	}

	/* add the local destination address */
	client_send_sge.addr = (uint64_t)dst;
	client_send_sge.length = (uint32_t)strlen(src);
	client_send_sge.lkey = client_dst_mr->lkey;
	bzero(&client_send_wr, sizeof(client_send_wr));

	/* create the worklist with the remote server address and READ operation */
	client_send_wr.sg_list = &client_send_sge;
	client_send_wr.num_sge = 1;
	...
	if (ret) {
		rdma_error("Failed to send read request, ret: %d\n", ret);
		return ret;
	}
	...
	if (ret != 1) {
		rdma_error("error when reading from remote, ret = %d\n", -errno);
		return -errno;
	}
